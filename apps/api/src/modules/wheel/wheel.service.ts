...
export class WheelService {
    ...
    async addItem(userId: string, setId: string, item: { name: string; color?: string | null; order: number }) {
        const set = await this.prisma.wheelSet.findFirst({ where: { id: setId, userId } });
        if (!set) throw new NotFoundException('Wheel set not found');
        return this.prisma.wheelItem.create({ data: { setId, name: item.name, color: item.color ?? null, order: item.order } });
    }

    async updateItem(userId: string, setId: string, itemId: string, data: { name?: string; color?: string | null; order?: number }) {
        const set = await this.prisma.wheelSet.findFirst({ where: { id: setId, userId } });
        if (!set) throw new NotFoundException('Wheel set not found');
        return this.prisma.wheelItem.update({
            where: { id: itemId },
            data: { name: data.name, color: data.color ?? undefined, order: data.order ?? undefined },
        });
    }
    ...
    async reorderItems(userId: string, setId: string, orders: { id: string; order: number }[]) {
        ...
        await this.prisma.$transaction(
            orders.map(o => this.prisma.wheelItem.update({ where: { id: o.id }, data: { order: o.order } })),
        );
    ...
    async importFromLocal(userId: string, payload: any) {
        ...
        for (const s of sets) {
            const created = await this.prisma.wheelSet.create({ data: { userId, name: String(s.name || '未命名') } });
